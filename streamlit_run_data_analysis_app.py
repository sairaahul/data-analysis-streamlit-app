...
# Main title
st.markdown('<h1 class="main-header">📊 Universal Data Analysis App</h1>', unsafe_allow_html=True)
...
with st.sidebar:
    ...
    # Only show clustering if sklearn is available
    if SKLEARN_AVAILABLE:
        show_clustering = st.checkbox("Clustering Analysis", value=False)
    else:
        show_clustering = False
        st.info("Install scikit-learn for clustering analysis")
...
def create_overview_metrics(df):
    ...
    with col1:
        ...
        <div class="metric-container">
            <h3 style="margin:0; color:#1f77b4;">📏 Rows</h3>
    ...
    with col2:
        st.markdown(f"""
        <div class="metric-container">
            <h3 style="margin:0; color:#1f77b4;">📊 Columns</h3>
    ...
    with col3:
        st.markdown(f"""
        <div class="metric-container">
            <h3 style="margin:0; color:#1f77b4;">🔢 Numeric</h3>
    ...
    with col4:
        ...
        st.markdown(f"""
        <div class="metric-container">
            <h3 style="margin:0; color:#1f77b4;">❌ Missing</h3>
...
def create_distribution_plots(df, numeric_cols, categorical_cols):
    ...
    if numeric_cols:
        ...
        if len(numeric_cols) > 1:
            cols_to_plot = numeric_cols[:8]  # Limit for performance
            fig = go.Figure()
...
def create_time_series_analysis(df, datetime_cols, numeric_cols):
    ...
    st.markdown('<div class="section-header">⏱️ Time Series Analysis</div>', unsafe_allow_html=True)
...
def generate_insights(df, numeric_cols, categorical_cols):
    ...
    if missing_data.sum() > 0:
        ...
        insights.append(f"⚠️ Column '{highest_missing}' has the most missing values ({missing_data[highest_missing]:,} or {missing_data[highest_missing]/len(df)*100:.1f}%)")
    ...
    if len(numeric_cols) > 1:
        try:
            ...
            if corr_pairs:
                ...
                insights.append(f"🔗 Strong correlation ({corr_val:.3f}) found between '{col1}' and '{col2}'")
        except:
            pass
    ...
    for col in categorical_cols[:2]:
        try:
            ...
            insights.append(f"📋 Column '{col}' has {unique_count} unique values. Most common: '{most_common}' ({most_common_pct:.1f}%)")
        except:
            pass
...
if uploaded_file is not None:
    ...
    if df is not None:
        ...
        with col3:
            if datetime_cols:
                st.info(f"**DateTime Columns ({len(datetime_cols)}):**\n" + ", ".join(datetime_cols[:5]) + ("..." if len(datetime_cols) > 5 else ""))
    ...
    <div style="text-align: center; padding: 3rem;">
        <h2>👋 Welcome to the Universal Data Analysis App!</h2>
        ...
        """ + ("""
        <div style="background-color: #fff3cd; padding: 1rem; border-radius: 8px; margin-top: 2rem;">
            <h4>📦 Required Libraries</h4>
            <p>Make sure you have installed: <code>streamlit pandas numpy plotly</code></p>
            <p>Optional for clustering: <code>scikit-learn</code></p>
        </div>
