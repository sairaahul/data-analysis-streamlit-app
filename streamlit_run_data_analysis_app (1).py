...
def generate_insights(df, numeric_cols, categorical_cols):
    ...
    if len(numeric_cols) > 1:
        ...
        if len(high_corr) > 0 and high_corr.iloc[0] > 0.8:
            pair = high_corr.index[0]
            insights.append(f"🔗 Strong correlation ({high_corr.iloc[0]:.3f}) found between '{pair[0]}' and '{pair[1]}'")
...
def create_advanced_visualizations(df, numeric_cols, categorical_cols):
    ...
    if len(numeric_cols) > 1:
        fig = px.scatter_matrix(
            df[numeric_cols[:4]],  # Limit to first 4 numeric columns
